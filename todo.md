...
- [x] Decide how the monthly data refresh should handle the three browser-only source packages. Neither of the two options as framed was right: seeding needs storage and credentials I do not have, and dropping the schedule throws away the one thing the run still proves — that the three URLs this repo pins are serving. The cron stays; `download_real_data.py --tolerate-unseeded` now exits 3 when *only* the human-supplied packages are absent, so `refresh-data.yml` stops cleanly with a run summary naming what to seed, while a rotted pinned URL still exits 1 and fails red. Before this, a documented, operator-blocked precondition produced a red run and a failure notification every month — an alarm nobody would read by the time it meant something. Seed the three where the runner can read them and the same job starts opening pull requests with no further change.
- [ ] Seed `bls_oesm25nat.zip`, `bls_ep_2024_2034_occupation.xlsx` and `isco_08_structure.xlsx` where the Actions runner can read them, if the monthly refresh should produce releases on its own rather than only checking the pinned URLs. Needs somewhere to put them (a release asset, an R2 bucket, a private repo) and the credentials to read it. The files themselves are operator-supplied by hand: the BLS blocks automated retrieval and their usage policy prohibits bot activity, so that step must not be scripted.
...
- [x] Stop deploying `data/world-mvt/<release>/manifest.json` (8.27 MiB build input) and `__manus__/debug-collector.js` (dev-only) as public assets, via `client/public/.assetsignore`. Verified live: with a cache-busting query both now return the 4,067-byte SPA shell instead of their bodies, while the derived 13,224-byte manifest, a real tile (76,811 B) and a `packed/**` part index all still serve.
- [ ] Purge Cloudflare's edge cache for `/data/world-mvt/world-global-deep-y-corrected-20260824/manifest.json`; needs account credentials. The asset is no longer deployed, but the canonical URL still serves the old 8,672,388-byte body from the edge — it had populated that entry under the `/data/world-mvt/:release/*` rule's `max-age=31536000, immutable`, so it can persist for up to a year. Inert (nothing requests it) but publicly fetchable until purged or expired.
