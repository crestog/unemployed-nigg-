...
const sourcePath = path.join(
  ...
  "public",
  "data",
  "world-india-geography.json"
);
const outputBase = path.join(root, "client", "public", "data", "india-tiles");
const source = JSON.parse(await readFile(sourcePath, "utf8"));
const outputRoot = path.join(outputBase, source.releaseId);
...
const coordinatePairs = geometry => {
  ...
  if (geometry.type === "GeometryCollection")
    return geometry.geometries.flatMap(coordinatePairs);
  return [];
};
const geometryBounds = geometry => {
  const pairs = coordinatePairs(geometry).filter(
    pair => finite(pair?.[0]) && finite(pair?.[1])
  );
...
const tileX = (longitude, z) =>
  clamp(Math.floor(((longitude + 180) / 360) * 2 ** z), 0, 2 ** z - 1);
...
const compactGeometry = (geometry, tolerance = 0) => {
  ...
  const pointDistance = (point, start, end) => {
    ...
    const t =
      ...
      (dx * dx + dy * dy);
    const projection =
      t < 0 ? start : t > 1 ? end : [start[0] + t * dx, start[1] + t * dy];
  ...
  const simplifyRing = ring => {
    ...
    const closed =
      simplified.length >= 3 ? [...simplified, simplified[0]] : ring;
...
for (const [layer, config] of Object.entries(layerConfig)) {
  ...
  const records =
    layer === "localities"
      ? config.source.records.map(record => ({
          ...record,
          _bounds: [
            record.longitude,
            record.latitude,
  ...
  for (const [key, recordsForTile] of layerTiles) {
    ...
    const payload = JSON.stringify({
      ...
      x: Number(x),
      y: Number(y),
      records: recordsForTile,
    });
    await writeFile(
      path.join(layerRoot, `${x}-${y}.json`),
      `${payload}\n`,
      "utf8"
    );
  }
...
const manifestPayload = JSON.stringify({
  ...
  baseUrl: `/data/india-tiles/${source.releaseId}`,
  sourceAsset: "/data/world-india-geography.json",
  coordinateSystem:
    "WGS84 longitude/latitude; geometry coordinates rounded to 5 decimals for display tiles",
  ...
  JSON.stringify(
    {
      ...
      layers: Object.fromEntries(
        [...tileMap].map(([layer, meta]) => [
          layer,
          {
            count: meta.count,
            tileZoom: meta.tileZoom,
