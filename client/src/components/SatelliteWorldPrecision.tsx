...
export default function SatelliteWorldPrecision({ records, selectedId, onSelect, view, onViewChange }: { records: LegalRecord[]; selectedId: string | null; onSelect: (id: string) => void; view: PrecisionView; onViewChange: (view: PrecisionView) => void }) {
  ...
  const onMapReady = useCallback((map: google.maps.Map) => {
    ...
    map.setOptions({ minZoom: 2, maxZoom: 20, disableDefaultUI: true, mapTypeControl: false, fullscreenControl: false, zoomControl: false, streetViewControl: false, keyboardShortcuts: false, gestureHandling: "greedy", clickableIcons: false });
    ...
    map.addListener("idle", () => {
      ...
      if (center && zoom != null) onViewChange({ lat: Number(center.lat().toFixed(6)), lng: Number(center.lng().toFixed(6)), zoom: Number(clamp(zoom, 2, 20).toFixed(2)) });
    });
    initializedRef.current = true;
    applyStyle();
  }, [applyStyle, onSelect, onViewChange]);
  ...
  return <div data-venture-overlay className="absolute inset-0 z-0 bg-[#20281f] select-none" onPointerDown={(event) => event.stopPropagation()} onWheel={(event) => event.stopPropagation()}>
    <MapView className="atlas-precision-map h-full w-full" initialCenter={{ lat: view.lat, lng: view.lng }} initialZoom={view.zoom} options={{ minZoom: 2, maxZoom: 20, disableDefaultUI: true, mapTypeControl: false, fullscreenControl: false, zoomControl: false, streetViewControl: false, keyboardShortcuts: false, gestureHandling: "greedy", clickableIcons: false }} onMapReady={onMapReady} />
    <div className="pointer-events-none absolute bottom-5 left-1/2 z-10 -translate-x-1/2 rounded-full border border-white/20 bg-[#172018]/90 px-4 py-2 font-mono text-[9px] uppercase tracking-[.13em] text-white/90 shadow-lg">Google hybrid visual context · Atlas overlays retain their own source and precision</div>
  </div>;
}
