...
export default function GlobalVentureAtlas() {
  ...
  useEffect(() => {
    ...
    if ([nextK, nextX, nextY].every(Number.isFinite)) setCamera({ k: clamp(nextK, 0.85, 6), x: nextX, y: nextY });
  }, []);
  useEffect(() => {
  ...
  useEffect(() => { if (!release) return; const timeout = window.setTimeout(() => { const params = new URLSearchParams(); params.set("world", "1"); params.set("wl", layer); params.set("wx", camera.x.toFixed(1)); params.set("wy", camera.y.toFixed(1)); params.set("wk", camera.k.toFixed(3)); if (selectedId) params.set("wc", selectedId); window.history.replaceState(null, "", `${window.location.pathname}${window.location.search}#${params.toString()}`); }, 260); return () => window.clearTimeout(timeout); }, [camera, layer, release, selectedId]);

  const selectCountry = (id: string) => { setSelectedId(id); setQuery(""); setShowResults(false); };
  const focusCountry = (id: string) => { selectCountry(id); const center = pathMaker.centroid(countryFeatures.find((item) => toTopologyId(item) === id) as GeoJSON.Feature); if (Number.isFinite(center[0]) && Number.isFinite(center[1])) setCamera({ x: size.width / 2 - (center[0] - size.width / 2) * 1.65, y: size.height / 2 - (center[1] - size.height / 2) * 1.65, k: 1.65 }); };
  const reset = () => { setCamera({ x: size.width / 2, y: size.height / 2, k: 1 }); setSelectedId(null); };
  ...
  const onWheel = (event: ReactWheelEvent<HTMLDivElement>) => { if (overlayTarget(event.target)) return; event.preventDefault(); const rect = event.currentTarget.getBoundingClientRect(); const px = event.clientX - rect.left; const py = event.clientY - rect.top; setCamera((current) => { const nextK = clamp(current.k * (event.deltaY < 0 ? 1.14 : 0.88), 0.85, 6); const mapX = (px - current.x) / current.k + size.width / 2; const mapY = (py - current.y) / current.k + size.height / 2; return { k: nextK, x: px - (mapX - size.width / 2) * nextK, y: py - (mapY - size.height / 2) * nextK }; }); };
  ...
  return <section ref={surfaceRef} className="relative min-h-[calc(100vh-68px)] overflow-hidden bg-[#fbfaf8]" onPointerDown={onPointerDown} onPointerMove={onPointerMove} onPointerUp={onPointerUp} onPointerCancel={() => setDrag(null)} onWheel={onWheel}>
    <svg className="absolute inset-0 h-full w-full touch-none" aria-label="World map of source-bounded legal entity and policy evidence" role="img" viewBox={`0 0 ${size.width} ${size.height}`}>
    ...
    <div data-venture-overlay className="absolute left-4 top-4 z-20 max-w-[min(630px,calc(100vw-2rem))] select-text" onPointerDown={(event) => event.stopPropagation()} onWheel={(event) => event.stopPropagation()}>
      ...
      <div className="mt-2 inline-flex rounded-full border border-[#d9d6ca] bg-[#fffefb]/95 p-1 shadow-sm backdrop-blur" role="tablist" aria-label="World evidence layers">
    ...
    <aside data-venture-overlay className="absolute left-4 top-[128px] z-20 hidden w-[310px] border border-[#d5d2c7] bg-[#fffefb]/94 p-4 shadow-sm backdrop-blur lg:block select-text" onPointerDown={(event) => event.stopPropagation()} onWheel={(event) => event.stopPropagation()}>
      ...
      <p className="mt-3 text-sm leading-6 text-[#5d6058]">Countries are fields, not containers of “everything.” Zoom to inspect the publisher and its exact inclusion rule.</p>
      <div className="mt-4 space-y-2 border-t border-[#e4e1d7] pt-3 font-mono text-[9px] uppercase tracking-[.12em] text-[#73766d]"><p><span className="mr-2 text-[#0f766e]">01</span>Teal = GLEIF legal-address coverage</p><p><span className="mr-2 text-[#ba7a48]">02</span>Ochre = policy source context</p><p><span className="mr-2 text-[#b95c78]">03</span>Coral = selected field or coverage limit</p></div>
    </aside>

    <div data-venture-overlay className="absolute bottom-4 left-4 z-30 w-[min(520px,calc(100vw-2rem))] select-text" onPointerDown={(event) => event.stopPropagation()} onWheel={(event) => event.stopPropagation()}>
    ...
    <div data-venture-overlay className="absolute bottom-4 right-4 z-20 flex flex-col gap-2" onPointerDown={(event) => event.stopPropagation()} onWheel={(event) => event.stopPropagation()}><button onClick={() => setCamera((current) => ({ ...current, k: clamp(current.k * 1.25, .85, 6) }))} className="grid h-10 w-10 place-items-center rounded-xl border border-[#d6d3c8] bg-[#fffefb]/95 text-[#51544d] shadow-sm hover:border-[#2f837e]" aria-label="Zoom in"><ZoomIn className="h-4 w-4" /></button><button onClick={() => setCamera((current) => ({ ...current, k: clamp(current.k * .8, .85, 6) }))} className="grid h-10 w-10 place-items-center rounded-xl border border-[#d6d3c8] bg-[#fffefb]/95 text-[#51544d] shadow-sm hover:border-[#2f837e]" aria-label="Zoom out"><ZoomOut className="h-4 w-4" /></button><button onClick={reset} className="grid h-10 w-10 place-items-center rounded-xl border border-[#d6d3c8] bg-[#fffefb]/95 text-[#51544d] shadow-sm hover:border-[#2f837e]" aria-label="Reset world view"><LocateFixed className="h-4 w-4" /></button></div>

    {selectedId && <aside data-venture-overlay className="absolute bottom-4 right-4 top-4 z-40 flex w-[min(400px,calc(100vw-2rem))] flex-col overflow-hidden rounded-2xl border border-[#d8d6cd] bg-[#fffefb]/96 shadow-[0_24px_70px_rgba(36,40,34,.18)] backdrop-blur-xl select-text sm:right-16" onPointerDown={(event) => event.stopPropagation()} onWheel={(event) => event.stopPropagation()}>
      ...
      <div className="flex-1 overflow-auto overscroll-contain p-5"><div className="flex items-center justify-between gap-3"><div className="flex items-center gap-2 font-mono text-[9px] uppercase tracking-[.16em] text-[#2f837e]"><SourceDot tone={layerInfo.tone} />{layerInfo.label}</div><span className="rounded-full bg-[#f0ece5] px-2 py-1 font-mono text-[8px] uppercase tracking-[.13em] text-[#777970]">{selectedRecord?.countryCode ?? "geometry"}</span></div><h2 className="mt-4 font-display text-4xl leading-[.94] text-[#242822]">{selectedName}</h2><p className="mt-4 text-sm leading-6 text-[#62645d]">{layerInfo.description}</p>
